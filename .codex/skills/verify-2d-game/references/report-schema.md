...
Merged under this key only when `--vlm <findings.json>` is passed. Strict schema: `<internal schema — validated by `loombridge verify --vlm`>`. **Never folded into `status`** — informs, does not gate.
...
Schema: `<internal schema — validated by `loombridge plan`>`; reference instance pattern: `<game>.acceptance.json`; types: `<internal types — see `loombridge verify --help`>`. Sections:
