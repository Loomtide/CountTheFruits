...
2. **Local registry/profile fixtures** (`~/.loombridge/asset-layer/registry/*.json`,
   `~/.loombridge/asset-layer/profiles/*.json`) — only for **tests or offline/air-gapped runs**.
...
```bash
loombridge-asset-prep \
  --project "$PWD" \
  --profile ~/.loombridge/asset-layer/profiles/2d-topdown-arena.json \
  --registry ~/.loombridge/asset-layer/registry/switchyard-2d.json \
  --name switchyard
```
