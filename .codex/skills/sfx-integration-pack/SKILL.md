...
(`<internal — see `loombridge --help`>`), and the verification gates read it. This
...
type + self-validating parser live in `<internal — see `loombridge --help`>`. A malformed/hand-edited
...
The gates (`<internal — see `loombridge --help`>`) read the parsed cue map to know WHICH cues matter, then grade
