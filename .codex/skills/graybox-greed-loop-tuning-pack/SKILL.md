...
`<internal — see `loombridge --help`>` (per-run summary + closed event
set), `.../personas.json` (Timid/Balanced/Greedy/Reckless with intent + envelope + calibration posture),
and `<internal — see `loombridge verify --help`>evidence-classes.ts`. Point at those for the field lists — do not
restate them here.
